...
// @author       You
// @match        http://thesfm.com/timlowjk/?pg=video-series&ar=awlist5189954&t=shane-test2
// @grant        none
// ==/UserScript==

(function() {
    $("body").prepend('<style>table {border-collapse: collapse;}table, th, td {border: 1px solid black;}#status-table{margin-left:auto;margin-right:auto;margin-top:25px;margin-bottom:10px;}</style>');

    var parsingData = read_cookie("peeps");
    if(parsingData==null)
    {
    ...
    $("body").append('<div><textarea id="csv"></textarea><p>Paste Your CSV Here</p><button id="start">Start</button><button id="readCookie">Read</button><button id="clear">Clear</button></div>');
    ...
    else
    {
       ...
       $("body").append('<div><h1 style="color:blue">Auto Submitting Emails</h1><table id="status-table"><thead><tr><th>Email</th><th>Name</th><th>Submitted</th><th>Remaining</th><th>Submitting In</th></tr><tr><td id="current-email">shane@mws.com</td><td id="current-name">Shane</td><td id="peeps-count">4</td><td id="peeps-left">18</td><td id="auto-timer">N/A</td></tr></thead></table><button id="clear">Cancel</button></div>');
       var randomDelay = Math.floor(Math.random() * 100000);
        var cancel = false;
        setTimeout(function(){
            if(cancel!=true)
            {
                submitForm(parsingData);
            }

        }, randomDelay);
        displayCountdown(randomDelay);
        populateDisplayTable(parsingData);
...
function displayCountdown(milliseconds)
{
    ...
    var counter = Math.floor(milliseconds/1000);
var interval = setInterval(function() {
    counter--;
   $("#auto-timer").html(String(counter)+" seconds");
    if(counter<6)
    {
        $("#auto-timer").css("color","red");
    }
    if (counter == 0) {
        $("#auto-timer").html("Now!");
        // Display a login box
...
function populateDisplayTable(peoples)
{
    var counter = 0;
     for (var key in peoples)
    {
      ...
      if(!obj.submitted)
      {
          var remainingRecords = (peoples.length - counter)-1;
          $("#current-email").html(obj.email);
          $("#current-name").html(obj.name);
          $("#peeps-left").html(String(remainingRecords));
          $("#peeps-count").html(String(counter));
          break;
      }
        counter++;
     }
...
function startAutoSubmission()
{
 ...
 var config = {
    ...
    var emailJson = Papa.parse(csv,config)
    var relevantInfo = [];

    for (var key in emailJson.data) {
    var obj = emailJson.data[key];
        var person = {email:obj.email, name:obj.First_Name, submitted: false};
        relevantInfo.push(person);
     }
    console.log(relevantInfo);
    bake_cookie("peeps", relevantInfo);
    submitForm(relevantInfo);

}

function submitForm(peoples)
{
    var counter = 0;
    var allDone = false;
    for (var key in peoples)
    {
      ...
      {
          allDone = false;
          $("#inf_field_FirstName").val(obj.name);
          $("#inf_field_Email").val(obj.email);
          peoples[counter].submitted = true;
          delete_cookie("peeps");
          bake_cookie("peeps", peoples);
          $("#landing_optinform").submit();
          break;
      }
       else
       {
    ...
    {
        delete_cookie("peeps");
         $(".top-head").append('<div><h1 style="color:green">All done!</h1></div>');
    }
